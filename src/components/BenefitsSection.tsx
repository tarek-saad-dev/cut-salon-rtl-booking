    ...
    <section className="relative py-20 md:py-28 bg-cut-black overflow-hidden">
      ...
      <div className="container px-4 relative z-10">
        <div className="text-center mb-10 md:mb-14">
          <p className="text-cut-gold font-heading font-bold text-sm tracking-widest mb-3">لماذا Cut Salon؟</p>
          ...
          {benefits.map((b) => (
            ...
            >
              <div className="w-12 h-12 md:w-14 md:h-14 rounded-xl bg-cut-gold/8 border border-cut-gold/15 flex items-center justify-center mx-auto mb-4 md:mb-5 group-hover:bg-cut-gold/12 transition-colors">
                <b.icon className="w-5 h-5 md:w-6 md:h-6 text-cut-gold" />
              </div>
              <h3 className="font-heading text-sm md:text-lg font-bold text-cut-ivory mb-1.5">{b.title}</h3>
              <p className="text-cut-ivory/65 text-xs md:text-sm leading-relaxed">{b.desc}</p>
            </div>
          ))}
        </div>
      </div>
