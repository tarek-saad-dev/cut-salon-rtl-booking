...
const HowItWorksSection = () => {
  return (
    <section className="relative py-20 md:py-28 bg-cut-black overflow-hidden">
      ...
      <div className="container px-4 relative z-10">
        <div className="text-center mb-10 md:mb-14">
          <p className="text-cut-gold font-heading font-bold text-sm tracking-widest mb-3">خطوات الحجز</p>
          ...
          {steps.map((step, i) => (
            <div key={step.title} className="relative text-center">
              ...
              <div className="w-20 h-20 rounded-full border border-cut-gold/20 bg-cut-surface flex items-center justify-center mx-auto mb-4 relative z-10 shadow-[0_0_30px_rgba(164,136,121,0.06)]">
                <step.icon className="w-8 h-8 text-cut-gold" />
              </div>
              <span className="text-cut-gold font-heading font-bold text-xs mb-1.5 block tracking-widest">الخطوة {step.number}</span>
              <h3 className="font-heading text-lg md:text-xl font-bold text-cut-ivory mb-1.5">{step.title}</h3>
              <p className="text-cut-ivory/65 text-xs md:text-sm">{step.desc}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-10 md:mt-14 max-w-xl mx-auto text-center">
          <div className="rounded-2xl border border-white/[0.06] bg-cut-surface px-6 py-4">
            <p className="text-cut-ivory/55 text-xs md:text-sm leading-relaxed">
              عند حضورك في الموعد قد يكون هناك انتظار بسيط من <span className="text-cut-gold font-bold">1 إلى 10 دقائق</span> كحد أقصى حتى يبدأ دورك.
            </p>
          </div>
        </div>
      </div>
