...
import { isCoreService } from "@/lib/bookingServiceGroups";
...
export default function BookingServiceCart({
  ...
  const alwaysVisible = Boolean(onContinue);
  ...
  const primaryService = useMemo(() => {
    return (
      selectedServices.find((s) => isCoreService(s)) ?? selectedServices[0] ?? null
    );
  }, [selectedServices]);

  const primaryPresentation = useMemo(() => {
    if (!primaryService) return null;
    return getServicePresentation(primaryService, lang);
  }, [primaryService, lang]);
  ...
  return (
    <div
      className={`
        ...
        bg-[var(--booking-bg)]/95 shadow-[0_-8px_28px_rgba(15,23,42,0.08)] backdrop-blur-sm
      ...
      <div className="px-4 md:px-5 py-3 space-y-2.5">
        {count === 0 && onContinue ? (
          <button
            type="button"
            disabled
            className="w-full min-h-12 rounded-xl bg-[var(--booking-accent)]/35 text-[var(--booking-text-secondary)] font-bold text-sm disabled:cursor-not-allowed"
            data-cart-continue
            data-cart-continue-disabled
          >
            {t("service.chooseToContinue")}
          </button>
        ) : null}
        ...
        {count > 0 ? (
          <>
            ...
            <button
              ...
              aria-controls={panelId}
              className="
                hidden w-full min-h-11 items-center gap-3 rounded-xl px-1 text-start md:flex
                focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--booking-accent)]
              "
              data-cart-toggle
            >
            ...
            {onContinue ? (
              <button
                ...
                onClick={onContinue}
                disabled={continueDisabled}
                className="hidden w-full min-h-11 rounded-xl bg-[var(--booking-accent)] py-3 text-sm font-bold text-white disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--booking-accent)] md:block"
                data-cart-continue
              >
        ...
        {onBack ? (
          <button
            type="button"
            onClick={onBack}
            className="hidden w-full min-h-11 rounded-xl border border-[var(--booking-border)] py-2.5 text-sm font-medium text-[var(--booking-text-secondary)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--booking-accent)] md:inline-flex md:items-center md:justify-center md:gap-2"
            data-cart-back
          >
            {dir === "rtl" ? (
