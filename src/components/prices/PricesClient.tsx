...
import { ArrowLeft, ArrowRight, ArrowUpLeft, Check, ChevronLeft, Scissors, Sparkles } from "lucide-react";
...
type Service = { id: string; name: Localized; subtitle?: Localized; price: number; from?: boolean };
...
function ServiceRow({ service, language, dark }: { service: Service; language: Language; dark: boolean }) {
  return <div className={`grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 border-b py-5 last:border-b-0 md:gap-8 md:py-6 ${dark ? "border-cut-warm-beige/20" : "border-cut-bronze/30"}`}><div className="min-w-0 text-start"><h3 className="text-[1.05rem] font-bold tracking-tight md:text-xl">{t(service.name, language)}</h3>{service.subtitle && <p className={`mt-1.5 text-sm leading-6 ${dark ? "text-cut-ivory/65" : "text-cut-black/65"}`}>{t(service.subtitle, language)}</p>}</div><div className="flex items-center gap-3 md:gap-5"><span aria-hidden="true" className={`hidden h-px w-8 bg-gradient-to-l md:block ${dark ? "from-cut-bronze/70 to-transparent" : "from-cut-bronze/80 to-transparent"}`} /><p className={`whitespace-nowrap rounded-md border px-3 py-2 text-base font-black tabular-nums md:px-4 md:text-lg ${dark ? "border-cut-bronze/35 bg-cut-bronze/10 text-cut-warm-beige" : "border-cut-bronze/45 bg-cut-warm-paper text-cut-burgundy"}`}>{formatPrice(service.price, language, { from: service.from })}</p></div></div>;
}

function PriceSection({ section, language }: { section: PriceSectionData; language: Language }) {
  const dark = section.theme === "dark";
  return <section id={section.id} data-sort={section.sortOrder} className={`relative isolate scroll-mt-32 overflow-hidden ${dark ? "bg-[linear-gradient(135deg,#050505_0%,#170406_100%)] text-cut-ivory" : "bg-cut-soft-ivory text-cut-black"}`}><div className={`absolute inset-x-0 top-0 h-px ${dark ? "bg-cut-warm-beige/20" : "bg-cut-bronze/35"}`} /><p aria-hidden className={`pointer-events-none absolute bottom-2 left-1/2 -translate-x-1/2 whitespace-nowrap font-heading text-[clamp(5rem,17vw,14rem)] font-black leading-none tracking-tight ${dark ? "text-cut-ivory/[0.025]" : "text-cut-burgundy/[0.035]"}`}>{t(section.watermark, language)}</p><div className="container relative z-10 px-5 py-20 md:px-8 md:py-28"><div className="grid gap-10 lg:grid-cols-[0.74fr_1.26fr] lg:gap-24"><div className="lg:pt-2"><div className="flex items-center gap-3"><span className={`h-px w-10 ${dark ? "bg-cut-bronze" : "bg-cut-burgundy"}`} /><p className={`font-display text-xs tracking-[0.3em] ${dark ? "text-cut-bronze" : "text-cut-burgundy"}`}>{t(section.label, language)}</p></div><h2 className={`mt-4 text-4xl font-black tracking-tight md:text-6xl ${language === "en" ? "font-editorial font-semibold" : ""}`}>{t(section.title, language)}</h2><p className={`mt-5 max-w-sm text-base leading-8 ${dark ? "text-cut-ivory/70" : "text-cut-black/70"}`}>{t(section.description, language)}</p></div><div className={`self-end border-t ${dark ? "border-cut-warm-beige/20" : "border-cut-bronze/30"}`}>{section.services.map((service) => <ServiceRow key={service.id} service={service} language={language} dark={dark} />)}</div></div></div></section>;
}
...
export default function PricesClient() {
  ...
  return <main className={`overflow-x-clip bg-cut-black ${ar ? "font-body" : "font-body"}`} dir={ar ? "rtl" : "ltr"}>
    ...
    {catalogSections.map((section) => <PriceSection key={section.id} section={section} language={language} />)}
