...
function ErrorState({ message, onRetry }: { message: string; onRetry: () => void }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 px-6 text-center" dir="rtl">
      ...
      <h3 className="text-cut-ivory text-base font-black mb-1">
        {isAuthError ? "يرجى تسجيل الدخول أولاً" : "تعذر تحميل بيانات CUT CLUB"}
      </h3>
      <p className="text-cut-ivory/40 text-sm leading-relaxed max-w-xs mb-6">
        {isAuthError
      ...
      {isAuthError ? (
        <Link
          ...
          className="inline-flex items-center gap-2 rounded-xl border border-cut-gold/30 bg-cut-gold/[0.08] px-5 py-2.5 text-sm font-bold text-cut-gold transition-all hover:bg-cut-gold/15 hover:border-cut-gold/50 active:scale-[0.97]"
        >
          تسجيل الدخول
        </Link>
      ) : (
...
function PageHeader({ accentColor, border, bg }: { accentColor: string; border: string; bg: string }) {
  return (
    <div className="mb-8 text-center">
      ...
      />
      <p className="text-cut-ivory/40 text-sm">كل زيارة ليك بتقربك من مكافآت وتجربة أرقى</p>
    </div>
  );
}
      ...
      <div className="relative z-10 mx-auto max-w-4xl px-4 pb-16 pt-24">
        ...
        {!loading && !error && data && (
          <div className="mb-6 text-center" dir="rtl">
            <h2 className="text-cut-ivory text-xl md:text-2xl font-bold">
          ...
          <div className="space-y-6">
            ...
            {data.nextReward.requiredPoints > 0 ? (
              ...
              <div
                ...
                dir="rtl"
              >
                <p className="text-cut-gold text-sm font-bold mb-1">كل المكافآت الأساسية متاحة لك الآن 🎉</p>
                <p className="text-cut-ivory/40 text-xs">استعرض مكافآتك واستخدمها في زيارتك القادمة</p>
              </div>
            )}
            ...
            {/* 9 — New client CTA */}
            {isNewClient && (
              <div className="rounded-2xl border border-[rgba(164,136,121,0.15)] bg-[#0d0d0d] p-6 text-center">
                <div className="relative mb-4 mx-auto w-fit">
                  <div className="flex h-16 w-16 items-center justify-center rounded-2xl border border-cut-gold/20 bg-cut-gold/[0.06] mx-auto">
                    <Scissors className="h-7 w-7 text-cut-gold" />
                  </div>
                </div>
                <h3 className="text-cut-ivory text-base font-black mb-1">أهلًا بيك في CUT CLUB</h3>
                <p className="text-cut-ivory/40 text-sm mb-5">أول زيارة ليك هتبدأ تجمع نقاطك وتفتح مكافآت حصرية</p>
                <button
                  onClick={handleBook}
                  className="inline-flex items-center gap-2 rounded-2xl bg-gradient-to-b from-cut-gold to-cut-gold/80 px-8 py-3 text-sm font-black text-cut-black shadow-[0_10px_32px_rgba(164,136,121,0.25)] transition-all hover:brightness-110 active:scale-[0.97]"
                >
            ...
            {!isNewClient && (
              <div className="pt-4 pb-2 text-center">
                <button
                  onClick={handleBook}
                  className="inline-flex items-center gap-2 rounded-2xl bg-gradient-to-b from-cut-gold to-cut-gold/80 px-10 py-3.5 text-sm font-black text-cut-black shadow-[0_10px_32px_rgba(164,136,121,0.22)] transition-all hover:brightness-110 hover:shadow-[0_14px_40px_rgba(164,136,121,0.35)] active:scale-[0.97]"
                >
                  <Calendar className="h-4 w-4" />
                  احجز زيارتك القادمة
                </button>
                <p className="text-cut-ivory/25 text-[11px] mt-3">
